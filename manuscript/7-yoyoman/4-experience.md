...
$$ E_A = \int\limits_{t=0}^T\|\pmb{\dot q}^+ - \pmb{\dot q}^-\|_M\delta+\pmb\tau\cdot\pmb{\dot q}dt $$ {#eq:energy}

Dans cette [@eq:energy], $\delta$ est l'impulsion de Dirac correspondant à l’impact, $\cdot$ est l’opérateur de produit
scalaire, et $\|\pmb x\| \triangleq \sqrt{\pmb x^\top M\pmb x}$.

Sur une pente d’angle $\alpha$, nous pouvons donc écrire le CoT suivant l’[@eq:cota].

$$ \text{CoT} = \sin\alpha + \int\limits_{t=0}^T\cfrac{\|\pmb{\dot q}^+
- \pmb{\dot q}^-\|_M\delta+\pmb\tau\cdot\pmb{\dot q}}{mgd}dt $$ {#eq:cota}
